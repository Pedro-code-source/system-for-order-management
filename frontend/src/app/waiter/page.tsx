...
import {
  Users, UtensilsCrossed, LogOut, Plus, Minus, Check,
  Clock, ShoppingCart, AlertCircle
} from 'lucide-react';
...
export default function WaiterPage() {
  ...
  const createOrder = async (e: React.FormEvent) => {
    ...
    await updateTable(selectedTable, { status: 'occupied' });
    setOrderItems([]);
                ...
                >
                  <div className="text-center">
                    ...
                    <select
                      ...
                      onChange={async (e) => {
                        e.stopPropagation();
                        await updateTable(table.id, { status: e.target.value as any });
                      }}
                      className={`w-full px-3 py-2 border-2 rounded-lg text-sm font-medium bg-white outline-none ${
                        table.status === 'available' ? 'border-green-300 text-green-700' :
        ...
        {activeTab === 'orders' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {inPersonOrders.filter(o => o.status !== 'completed' && o.status !== 'cancelled').map((order) => {
                ...
                return (
                  <div key={order.id} className="bg-white rounded-xl p-6 border border-gray-200 shadow-sm">
                    <div className="flex items-start justify-between mb-4">
                      <div>
                        <div className="text-lg font-bold text-gray-900">
                          Mesa {table?.number || order.tableNumber}
                        </div>
                    ...
                    <div className="space-y-2">
                      <select
                        value={order.status}
                        onChange={async (e) => await updateOrder(order.id, { status: e.target.value as any })}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm font-medium bg-white outline-none focus:ring-2 focus:ring-green-500"
                      >
                      ...
                      {order.status === 'completed' && (
                        <button
                          onClick={async () => {
                            if (order.tableId) {
                              await updateTable(order.tableId, { status: 'available' });
                            }
                          }}
            ...
            {inPersonOrders.filter(o => o.status !== 'completed' && o.status !== 'cancelled').length === 0 && (
              <div className="bg-white rounded-xl p-12 text-center border border-gray-200">
